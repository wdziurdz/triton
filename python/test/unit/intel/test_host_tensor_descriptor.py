...
# One-head tile of a contiguous (TOKENS, HEADS, HEAD_DIM) tensor. The (BLOCK_M, 1,
# BLOCK_D) block spans dimensions 0 and 2, so 2D block I/O is reachable only if the unit
# middle dimension is collapsed. That is sound when the tile fits one head (issues/7679)
# and unsound otherwise, where it would read the next head instead of zero-padding
# (issues/7464).
...
def test_host_descriptor_rank3_unit_middle_dim(HEAD_DIM, collapse, device):
    ...
    # Checking the descriptor rank rather than `tt.reshape`: a rank-reducing
    # descriptor_load also leaves no reshape behind yet is not collapsed.
