...
class XPUBackend(BaseBackend, metaclass=XPUBackendMeta):
    ...
    def get_tensordesc_specialization(arg, **kwargs):
        ...
        if getattr(arg, "padding", None) == "nan":
            key += "N"
        # 4 is all MaterializeBlockPointer's alignment check needs, but collapsing a
        # unit dimension of a rank-3 descriptor needs `shape[i] % block_shape[i] == 0`,
        # which 4 can never prove. Raise the cap to the block extent for that shape
        # only, so every other descriptor keeps its specialization key (issues/7679).
